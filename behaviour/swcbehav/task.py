...
images, some of which are "changes" (a new image) and some of which are
...
* Each image is 750 ms (250 ms image + 500 ms gray), as in the paper.
* Change times are drawn from a geometric distribution, as in the paper.
* 5% of images are omitted, except change images and the image immediately
...
IMAGE_DURATION = 0.75  # seconds per image presentation (250 ms image + 500 ms gray)
...
def make_session(
    ...
    Parameters
    ----------
    n_images : number of images in the session (paper: ~4800 == 1 hour).
    n_image_ids : size of the image set (paper: 8 familiar images).
    change_p : per-image change hazard. Run lengths are ``min_run + Geometric(change_p)``,
        so the mean run length is ``min_run + 1 / change_p``.
    min_run : minimum number of repeats before a change can occur.
    omission_prob : probability an eligible image is omitted.
    seed : RNG seed for reproducibility.

    Returns
    -------
    pandas.DataFrame with one row per image and columns:
    ...
    # Omissions: not on changes, and not on the image right before a change.
    ...
    return pd.DataFrame(
        {
            ...
            "time": np.arange(n) * IMAGE_DURATION,
            "image_id": image_ids,
            "is_change": is_change,
